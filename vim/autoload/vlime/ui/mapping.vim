if !exists('g:vlime_default_mappings')
    ...
    let g:vlime_default_mappings = {
                \ 'lisp': [
                    \ ['n', '<LocalLeader>?', ':call vlime#ui#ShowQuickRef("lisp")<cr>', 'Show this quick reference.'],
                    \
                    \ ['i', '<space>', '<space><c-r>=vlime#plugin#VlimeKey("space")<cr>', 'Trigger the arglist hint.'],
                    \ ['i', '<cr>', '<cr><c-r>=vlime#plugin#VlimeKey("cr")<cr>', 'Trigger the arglist hint.'],
                    \ ['i', '<tab>', '<c-r>=vlime#plugin#VlimeKey("tab")<cr>', 'Trigger omni-completion.'],
                    \
                    \ ['n', '<LocalLeader>cc', ':call vlime#plugin#ConnectREPL()<cr>', 'Connect to a server.'],
                    \ ['n', '<LocalLeader>cs', ':call vlime#plugin#SelectCurConnection()<cr>', 'Switch connections.'],
                    \ ['n', '<LocalLeader>cd', ':call vlime#plugin#CloseCurConnection()<cr>', 'Disconnect.'],
                    \ ['n', '<LocalLeader>cR', ':call vlime#plugin#RenameCurConnection()<cr>', 'Rename the current connection.'],
                    \
